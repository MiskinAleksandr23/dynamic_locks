...
namespace {
  ...
  ScenarioInput MakeShiftRandomRangeScenario() {
    return {
      "shift_hotspot_random_ranges",
      "right warmup, then left hotspot; final ranges have random length",
      {
        {"warmup right hotspot random ranges",
         BuildQueries(kRandomRangeWarmupQueries, QueryPattern::kHotWindowRandomRange,
                      TrafficSide::kRight, kRandomRangeMaxLength, 31)},
        {"adapt left hotspot random ranges",
         BuildQueries(kRandomRangeAdaptQueries, QueryPattern::kHotWindowRandomRange,
      ...
      {"measure left hotspot random ranges",
       BuildQueries(kRandomRangeMeasuredQueries,
                    ...
                    TrafficSide::kLeft, kRandomRangeMaxLength, 33)},
      true
    };
  }
  ...
  ScenarioInput MakeRandomRangeScenario() {
    return {
      "random_uniform_ranges",
      "uniform random ranges over the whole array; range length is random",
      {
        {"warmup uniform random ranges",
         BuildQueries(kRandomRangeWarmupQueries, QueryPattern::kUniformRandomRange,
                      TrafficSide::kLeft, kRandomRangeMaxLength, 41)}
      },
      {"measure uniform random ranges",
       BuildQueries(kRandomRangeMeasuredQueries,
                    QueryPattern::kUniformRandomRange,
                    TrafficSide::kLeft, kRandomRangeMaxLength, 42)},
      false
    };
  }
  ...
  template<typename Lock>
  void WaitForBackgroundRebuild(Lock &, double &) {
  }
  ...
  template<typename Lock>
  double FlushPendingTraining(Lock &lock) {
    const auto start = std::chrono::steady_clock::now();
  ...
  ImplementationResult RunImplementation(const std::string &name,
...

    {
      ...
      result.setup_seconds =
          RunSetup(lock, data, scenario, QueryBody::kRealWork);
      ...
      const PhaseRunStats measured =
          RunPhase(lock, data, scenario.measured_phase.queries,
                   QueryBody::kRealWork);
  ...
  void PrintScenario(const ScenarioInput &scenario,
    ...
    std::cout << ", measured queries: "
        << scenario.measured_phase.queries.size() << "\n\n";

    std::cout << "  impl       setup_s  body_s  body_x  lock_only_s  lock_x"
    ...
    for (const ImplementationResult &result: results) {
      const double body_speedup =
          result.body_seconds > 0.0
            ? baseline.body_seconds / result.body_seconds
            : 0.0;
      ...
      std::cout << "  " << std::left << std::setw(10) << result.name
          ...
          << std::setw(8) << result.setup_seconds
          << std::setw(8) << result.body_seconds
          << std::setw(8) << body_speedup
          << std::setw(13) << result.lock_only_seconds
          << std::setw(8) << lock_speedup
          << std::setw(13) << std::setprecision(2) << result.avg_lock_us
          << std::setw(12) << std::setprecision(3)
          << result.measured_lock_total_seconds
          << std::setw(11) << std::setprecision(2)
          << result.avg_mutexes_per_query
          << std::setw(5) << result.left_hot_locks << '/'
          << std::setw(2) << result.right_hot_locks
          << std::setw(10) << result.reconfigurations << '\n';
    }
    std::cout << '\n';
  }
  ...
  void RunScenario(const ScenarioInput &scenario) {
    ...
    auto naive_factory = [] {
      return NaiveLock<kLockCount>(kArraySize);
    };
    auto dynamic_factory = [] {
      return DynamicLock<kLockCount>(kArraySize);
    };
...
int main() {
  ...
  std::cout << "  lock_only_s: same final coordinates after same setup, empty body\n";
  std::cout << "  body_x and lock_x are speedups over naive in the same scenario\n\n";
