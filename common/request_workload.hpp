...
public:
  WorkloadGenerator(size_t array_size, size_t hot_window_size,
      ...
      : array_size_(array_size), hot_window_size_(hot_window_size),
        query_length_(query_length), rng_(seed) {}
  ...
  size_t RandomColdWindowStart(size_t excluded_hot_begin) {
    if (excluded_hot_begin == 0) {
      std::uniform_int_distribution<size_t> dist(
          hot_window_size_, array_size_ - hot_window_size_);
      return dist(rng_);
    }

    std::uniform_int_distribution<size_t> dist(0, array_size_ -
                                                      2 * hot_window_size_);
    return dist(rng_);
  }
