#ifndef VERLIB_BTREE_ADAPTER_H
#define VERLIB_BTREE_ADAPTER_H
...
public:
    ...
    void printSummary() {
        // std::cout << "Verlib btree summary" << std::endl;
        // tree->print();
    }
    ...
    public:
        ...
        public:
            ChildIterator(NodePtrType _node) 
                : node(_node), current(0), 
                  size(_node ? _node->size : 0) {}
            ...
            NodePtrType next() {
                return node ? node->children[current++].load() : nullptr;
            }
        };
        ...
        size_t getNumKeys(NodePtrType node) {
            if (!node || !node->is_leaf) return 0;
            return static_cast<typename ordered_map<K,V>::leaf*>(node)->size;
        }
        
        size_t getSumOfKeys(NodePtrType node) {
            if (!node || !node->is_leaf) return 0;
            auto leaf = static_cast<typename ordered_map<K,V>::leaf*>(node);
            size_t sum = 0;
