...
struct log_array {
  ...
  void init() { // TODO: can this just be a constructor?
    for (int i=0; i < Log_Len; i++) {
      if (log_entries[i].load() != nullptr) 
        log_entries[i].store(nullptr, std::memory_order_relaxed);
    }
  ...
  ~log_array() {
    if(next != nullptr) {
      get_log_array_pool().destruct(next);
    }
  }
};
  ...
  log_entry* next_entry() {
    ...
    if (count == Log_Len) { // ran out of slots
      ...
      if (next_log_array != nullptr) vals = next_log_array;
        ...
        log_array* new_log_array = get_log_array_pool().new_obj();
        new_log_array->init();
        if(vals->next.compare_exchange_strong(next_log_array, new_log_array))
          ...
          vals = next_log_array;
          get_log_array_pool().destruct(new_log_array);
        }
      }
    }
    return &(*vals)[count++];
...
bool skip_if_done_no_log(F f) {
  ...
  if (l->load() == nullptr) { // check that not already completed
    f();
    // mark as completed
    l->store((void*) 1, std::memory_order_release);
    return true;
  }
  return false;
}
...
bool skip_if_done(F f) {
  ...
  if (v == nullptr) { // check that not already completed
    ...
    l->store((void*) x, std::memory_order_release);
    return true;
  }
